...
module.exports = class PacketStream extends Transform {
    ...
    _parsePacket (next) {
        ...
        if (!this._length) {
            ...
            if (this._debug) {
                // eslint-disable-next-line
                console.log('_parsePacket  lengths', this.Lengths)
            }
        ...
        var raw = this._buffer.slice((this._withPrefix ? 0 : this._prefixLength), this._prefixLength + this._length)

        this._buffer = this._buffer.slice(this._prefixLength + this._length)
        this._length = null
